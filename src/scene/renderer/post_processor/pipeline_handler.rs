...
use crate::scene::{
    ...
    renderer::{
        ...
        post_processor::{
            ...
            shader_preprocessor::{self, EffectLayout, WM_SAMPLER_BINDING},
        },
    },
};
...
fn create_effect_pipeline(
    ...
    let effect_json: Value =
        serde_json::from_str(scene.jsons.get(effect_path)?).ok()?;

    let material_path = effect_json["passes"][0]["material"].as_str()?;

    let material_json: Value =
        serde_json::from_str(scene.jsons.get(material_path)?).ok()?;
    ...
    let mut defines = collect_default_defines(vert_source, frag_source);

    apply_texture_combos(&mut defines, pass_textures);
    ...
    let effect_bgl = create_effect_bindgroup_layout(device, &layout);
...
fn apply_texture_combos(
    defines: &mut BTreeMap<String, String>,
    pass_textures: &[Option<String>],
) {
    if pass_textures.first().and_then(|t| t.as_deref()).is_some() {
        defines.entry("MASK".to_string()).or_insert_with(|| "1".to_string());
    }
    if pass_textures.get(1).and_then(|t| t.as_deref()).is_some() {
        defines.entry("TIMEOFFSET".to_string()).or_insert_with(|| "1".to_string());
    }
}

fn collect_default_defines(vert_source: &str, frag_source: &str) -> BTreeMap<String, String> {
...
fn create_effect_bindgroup_layout(device: &Device, layout: &EffectLayout) -> BindGroupLayout {
